...
class PostDetail extends PureComponent {
  ...
  renderContent = postList => {
    ...
    if (typeof index === "undefined" || !postList || !postList.length || postList.length == 0 || Platform.OS == "android") {
      return (
        <PostDetailView
          post={post}
          onBack={onBack}
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (nextProps.index !== this.props.index) {
      this.flatlist && setTimeout(() => {this.flatlist.scrollToOffset({ offset: nextProps.index * width })}, 100)
    }
  }
